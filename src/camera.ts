import { Mat4 } from "../node_modules/wgpu-matrix/dist/2.x/mat4";
import { Vec3 } from "../node_modules/wgpu-matrix/dist/2.x/vec3";
import { mat4, vec3 } from "../node_modules/wgpu-matrix/dist/2.x/wgpu-matrix";
...
export class FPCamera {
	...
	tick(deltaTime: number) {
		...
		if (this.inputs[2])
			vec3.add(movement_vec, movement_vec, vec3.scale(this.forward, -1));
		if (this.inputs[3])
			vec3.add(movement_vec, movement_vec, vec3.scale(this.right, -1));
		if (this.inputs[4])
			vec3.add(movement_vec, movement_vec, this.up);
		if (this.inputs[5])
			vec3.add(movement_vec, movement_vec, vec3.scale(this.up, -1));

		vec3.normalize(movement_vec, movement_vec);
		vec3.add(this.position, this.position, vec3.scale(movement_vec, this.speed * deltaTime))
	}
	...
	screen_to_ray(x: number, y: number, sx: number, sy: number): Ray {
		...
		vec3.add(ray_direction, ray_direction, vec3.scale(this.right, horizontal_coefficient));
		vec3.add(ray_direction, ray_direction, vec3.scale(this.up, vertical_coefficient));
